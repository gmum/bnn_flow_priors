...
from bnn_priors import data as bnn_priors_data
...
# TODO what is _run? (seems to be a local argument from other functions)
# if save_samples:
...
for e in range(NEPOCHS):
    ...
    for x, y in dataloader:
        ...
        qs, posterior_samples = sample_posterior(NSAMPLES_LEARNING, only_pointwise_locs=not learn_distributions)
        prior_samples = {name: buffer.repeat(NSAMPLES_LEARNING) for name, buffer in model.named_buffers()}
    ...
    if e % 10 == 0 and e > 0:
        qs, posterior_samples = sample_posterior(NSAMPLES_TESTING)
        prior_samples = {name: buffer.repeat(NSAMPLES_TESTING) for name, buffer in model.named_buffers()}
